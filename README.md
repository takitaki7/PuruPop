...
- **デイリーチャレンジ**：HUD のカレンダーボタンから、**世界中の全プレイヤーが同じ盤面**に挑戦できます（日付をシードにした決定論的生成なので、同じ日なら必ず同じ問題）。難易度はキャンペーン進捗に関係なく固定8色。クリアすると Wordle 風のシェアテキストをコピー／共有でき、絵文字グリッドは**盤面ではなく「自分がどの順番で色を完成させたか」**を表すので、同じ日でも人によって違う並びになります。連続クリア日数（ストリーク）も記録。未プレイの日はボタンに金色のドットが付きます。
